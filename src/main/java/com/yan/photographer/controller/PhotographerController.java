package com.yan.photographer.controller;
...
import com.yan.common.util.OperationFileUtil;
import com.yan.model.TPhotographer;
import com.yan.model.TPhotographerSpots;
import com.yan.photographer.service.IPhotographerService;
import com.yan.photographer.service.IPhotographerSpotsService;
import com.yan.vo.PageVO;
import com.yan.vo.ReturnCodeType;
import com.yan.vo.ReturnResult;
...
public class PhotographerController {
	...
	@ResponseBody
	public ReturnResult addPhotographer(TPhotographer photographer, HttpServletRequest request,
			 Integer spotsId) {
	...
	@ResponseBody
	public ReturnResult updatePhotographer(TPhotographer photographer,
			Integer spotsId) {
		returnResult.setStatus(ReturnCodeType.FAILURE);
		try {
		...
		try {
			returnResult.setStatus(ReturnCodeType.SUCCESS)
					.setData(photographerService.selectBySQL(
							"SELECT a.*,d.spotsId FROM t_photographer a,t_photographer_spots d WHERE a.id ="
									+ id + " AND d.photographerId=" + id + "")
							.get(0));
		} catch (Exception e) {
	...
	public ReturnResult getPhotographer(Integer id) {
		...
		try {
			returnResult.setStatus(ReturnCodeType.SUCCESS)
			.setData(photographerService.selectBySQL(
					"SELECT a.name,a.head,a.summary,e.`name` AS spots FROM t_photographer a,t_photographer_spots c,t_spots e WHERE a.id="+id+" AND c.photographerId="+id+" AND e.id = c.spotsId AND a.`status`=0")
					.get(0));
		} catch (Exception e) {
			logger.error("根据id获取Photographer失败" + e);
		}
	...
	@ResponseBody
	public ReturnResult getPhotographerListByPage(PageVO page, String spotsId,
			String beforeTime, String afterTime, String status, String name) {
		returnResult.setStatus(ReturnCodeType.FAILURE);
		try {
			Map<String, Object> resultMap = new HashMap<String, Object>();
			StringBuffer sql = new StringBuffer(
					"SELECT a.*,h.`name` AS spots FROM t_photographer a,t_photographer_spots d,t_spots h WHERE a.id = d.photographerId AND h.id= d.spotsId");
	...
	@ResponseBody
	public ReturnResult getPhotographerListByPageStatus(PageVO page, String spotsId,
			String start, String end) {
		returnResult.setStatus(ReturnCodeType.FAILURE);
		try {
			Map<String, Object> resultMap = new HashMap<String, Object>();
			StringBuffer sql = new StringBuffer(
					"SELECT a.*,h.`name` AS spots FROM t_photographer a,t_photographer_spots d,t_spots h WHERE a.id = d.photographerId AND h.id= d.spotsId AND a.status=0");

			StringBuffer countSql = new StringBuffer("SELECT COUNT(*) AS total FROM t_photographer a,t_photographer_spots d,t_spots h WHERE a.id = d.photographerId AND h.id= d.spotsId AND a.status=0");
	...
	public ReturnResult getAllPhotographer() {
		...
		try {
			returnResult.setStatus(ReturnCodeType.SUCCESS).setData(photographerService.selectBySQL(
					"SELECT a.id,a.name FROM t_photographer a,t_photographer_spots d,t_spots h WHERE a.id = d.photographerId AND h.id= d.spotsId AND a.status=0"));
		} catch (Exception e) {
